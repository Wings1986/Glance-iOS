...
- (void)viewDidLoad {
    ...
    [thumbImgView setImage:self.thumbImage];
    ...
    [geocoder reverseGeocodeLocation:newLocation
                   completionHandler:^(NSArray *placemarks,
                       ...
                       if (placemarks && placemarks.count > 0)
                       {
                           ...
                           locInfoView.text = [NSString stringWithFormat:@"%@, %@", [addressDictionary objectForKey:@"Country"], [addressDictionary objectForKey:@"City"]];
...
- (IBAction)SetCategory:(id)sender{
    [categoryPickerView setHidden:NO];
}
...
- (IBAction)Done:(id)sender{
    ...
    [self postResource:@"" andMethod:@"POST" andWithParams:@{@"lat": latString,
                                                             @"lng": lonString,
                                                             @"headline": headlineTxtField.text,
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    return [[[[AppDelegate sharedInstance] getDataFromPlist] objectForKey:@"Category"] count];
}

- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    NSString *resultString = [[[AppDelegate sharedInstance] getDataFromPlist] objectForKey:@"Category"][row];
    return resultString;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    categoryLabel.text = [NSString stringWithFormat:@"Category: %@", category];
